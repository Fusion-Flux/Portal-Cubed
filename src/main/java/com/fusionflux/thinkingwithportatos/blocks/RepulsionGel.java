                ...
                if (!entity.isSneaking() && (entity.verticalCollision || entity.horizontalCollision)) {
                    ...
                    //if (limiter.check(world, entity)) {
                        if ( !direction.equals(new Vec3d(0, 0, 0))) {
                            if(world.isClient) {
                                if (limiter.check(world, entity)) {
                                    world.playSound((PlayerEntity) entity, entity.getPos().getX(), entity.getPos().getY(), entity.getPos().getZ(), ThinkingWithPortatosSounds.GEL_BOUNCE_EVENT, SoundCategory.BLOCKS, .3F, 1F);
                                }
                                }else {
                                world.playSound(null, entity.getPos().getX(), entity.getPos().getY(), entity.getPos().getZ(), ThinkingWithPortatosSounds.GEL_BOUNCE_EVENT, SoundCategory.BLOCKS, .3F, 1F);
                            }
