        ...
        if (!world.isClient) {
            ...
            if (redstonePowered) {
                ...
                if (blockEntity.extensionTicks <= blockEntity.EXTENSION_TIME) {
                    if (world.isAir(blockEntity.obstructorPos) || world.getBlockState(blockEntity.obstructorPos).getHardness(world, blockEntity.obstructorPos) <= 0.1F || world.getBlockState(blockEntity.obstructorPos).getBlock().equals(PortalCubedBlocks.HLB_BLOCK)) {
    ...
    private void extendBridge(BlockState state, ServerWorld world, BlockPos pos) {
        ...
        for (int i = 0; i < BLOCKS_PER_TICK; i++) {
            if (extensionTicks <= EXTENSION_TIME) {
                if (world.getBlockState(extendPos).getBlock().equals(PortalCubedBlocks.HLB_BLOCK)) {
                    ...
                    world.setBlockState(extendPos, PortalCubedBlocks.HLB_BLOCK.getDefaultState().with(Properties.FACING, facing));
                    ((HardLightBridgeBlockEntity) Objects.requireNonNull(world.getBlockEntity(extendPos))).emitters.add(new BlockPos.Mutable(getPos().getX(), getPos().getY(), getPos().getZ()));
                } else {
                    bridgeComplete = true;
                    shouldExtend = false;
        ...
        if (!world.isClient) {
            ...
            while (!maintainPos.equals(obstructorPos)) {
                ...
                if (world.isAir(maintainPos)) {
                    world.setBlockState(maintainPos, PortalCubedBlocks.HLB_BLOCK.getDefaultState().with(Properties.FACING, facing), 3);
                ...
                else if (!world.getBlockState(maintainPos).getBlock().equals(PortalCubedBlocks.HLB_BLOCK)) {
                    shouldRetract = true;
                    break;
            ...
            if (shouldRetract) {
                ...
                while (!obstructorPos.equals(maintainPos)) {
                    ...
                    if (world.getBlockState(obstructorPos).getBlock().equals(PortalCubedBlocks.HLB_BLOCK)) {
