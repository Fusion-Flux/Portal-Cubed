...
public class GelOrbEntity extends ThrownItemEntity {
    ...
    public GelOrbEntity(World world) {
        super(PortalCubedEntities.GEL_ORB, 0, 0, 0, world);
    }

    public GelOrbEntity(World world, LivingEntity owner) {
        super(PortalCubedEntities.GEL_ORB, owner, world);
    }

    public GelOrbEntity(World world, double x, double y, double z) {
        super(PortalCubedEntities.GEL_ORB, x, y, z, world);
    }
        ...
        if (blockState.isAir() || blockState.isOf(PortalCubedBlocks.PROPULSION_GEL)) {
            if (this.world.getBlockState(hitBlock).isFullCube(this.world, getBlockPos())) {
                ...
                if (!this.world.getBlockState(hitPos.add(cardinalDir)).isAir()) {
                    ...
                    if (blockState.isAir()) {
                        initialState = PortalCubedBlocks.PROPULSION_GEL.getDefaultState();
                    } else {
