...
import com.fusionflux.portalcubed.packet.EntitySpawnPacket;
...
public class RepulsionGelOrbEntity extends ThrownItemEntity {
    ...
    public RepulsionGelOrbEntity(World world) {
        super(PortalCubedEntities.REPULSION_GEL_ORB, 0, 0, 0, world);
    }

    public RepulsionGelOrbEntity(World world, LivingEntity owner) {
        super(PortalCubedEntities.REPULSION_GEL_ORB, owner, world);
    }

    public RepulsionGelOrbEntity(World world, double x, double y, double z) {
        super(PortalCubedEntities.REPULSION_GEL_ORB, x, y, z, world);
    }
    ...
    protected void onCollision(HitResult hitResult) { // called on collision with a block
        ...
        if (blockState.isAir() || blockState.isOf(PortalCubedBlocks.PROPULSION_GEL)) {
            if (this.world.getBlockState(hitBlock).isFullCube(this.world, getBlockPos())) {
                ...
                if (!this.world.getBlockState(hitPos.add(cardinalDir)).isAir()) {
                    ...
                    if (blockState.isAir()) {
                        initialState = PortalCubedBlocks.GEL.getDefaultState();
                    } else {
    ...
    public Packet createSpawnPacket() {
        return EntitySpawnPacket.create(this, PortalCubedClientPackets.SPAWN_PACKET);
    }
}
